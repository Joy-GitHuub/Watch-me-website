...
import MoreShowImage from './../MoreShowImage/MoreShowImage';
...
const UserInfo = () => {
    ...
    const getUserInfo = async (id) => {
        const url = `https://watch-me-server-gamma.vercel.app/api/watch-me/v1/user/userInfo/${id}`
...
<div className='log_out_btn'>
    {/* <button>Log-Out</button> */}
        ...
        {getUser?.imagePost ?
           <>
            <section className='userInfo_banner_section'>
        </section>
        <section className='userInfo_header_section'>
            <div className="userInfo_header_wrap">
                <img src={getUser?.photoURL} alt="" />
                <h3>{getUser?.displayName}</h3>
            </div>

            <div className='userInfo_created_saved_btn_wrap'>
                <button
                onClick={(e) => {
                    setText(e.target.textContent)
                    setActiveBtn('created');
                }}
                 className={`${activeBtn === 'created' ? 'active_btn': ''}`}>Created</button>
                <button
                onClick={(e) => {
                    setText(e.target.textContent)
                    setActiveBtn('saved');
                }}
                className={`${activeBtn === 'saved' ? 'active_btn': ''}`}
                >Saved</button>
            </div>
        </section>

        {text === "Created" ? <section className='userInfo_photo_container'>
        <div className="show_image_container_wrap">
        {
            getUser?.imagePost?.map((image, index) => <MoreShowImage
            key={index}
            image={image}
            getUser={getUser}
            ></MoreShowImage>)
